 ...
 * Proxy seguro: reenvía el body a Power Automate usando la URL
 * almacenada en la variable de entorno del servidor.
 * El frontend NUNCA ve el parámetro `sig`.
 */
export default async function handler(req: Request, res: Response) {
  ...
  if (!paUrl) {
    return res.status(500).json({ ok: false, error: 'POWER_AUTOMATE_WEBHOOK_URL no configurado' });
  }
  ...
  try {
    ...
    console.error('api/submit-enrollment proxy error:', err);
