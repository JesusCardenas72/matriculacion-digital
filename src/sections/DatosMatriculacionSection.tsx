...
import { FileText, AlertCircle, CheckCircle2, GraduationCap } from 'lucide-react';
import { EnrollmentFormData } from '../types';
import { Materia } from '../data/materias';

interface DatosMatriculacionSectionProps {
  formData: EnrollmentFormData;
  onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => void;
  cursos: string[];
  asignaturasCursoActual: Materia[];
  asignaturasPrevias: { id: string; label: string; materiaId: string }[];
...
export const DatosMatriculacionSection: React.FC<DatosMatriculacionSectionProps> = ({
  formData,
  onChange,
  cursos,
    ...
    <section className="bg-white rounded-[2rem] p-4 sm:p-6 md:p-8 shadow-sm border border-gray-100">
      <div className="flex items-center justify-between mb-3 sm:mb-6">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${convalidacionAsignaturas.length > 0 ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}
        >
          {convalidacionAsignaturas.length > 0 && <CheckCircle2 size={16} />}
          Convalidación
        </button>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
          <div className="space-y-1">
            ...
            <select required name="curso" value={formData.curso} onChange={onChange} className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-gray-200 transition-all appearance-none cursor-pointer">
          ...
          <div className="md:col-span-2 space-y-1">
            ...
            <select name="asignaturaPendiente1" value={formData.asignaturaPendiente1} onChange={onChange} className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-gray-200 transition-all appearance-none cursor-pointer">
          ...
          {formData.tipoEnsenanza === 'profesional' && (
            <div className="md:col-span-2 space-y-1">
              ...
              <select name="asignaturaPendiente2" value={formData.asignaturaPendiente2} onChange={onChange} className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-gray-200 transition-all appearance-none cursor-pointer">
                <option value="">Ninguna...</option>
                {asignaturasPrevias.map(a => (
                  <option key={a.id} value={a.id}>
                    {a.materiaId} - {a.label}
                ...
                {(asignaturasCursoActual.length > 0 || selectedPendingSubjects.length > 0) ? (
                  <div className="p-6 bg-blue-50 rounded-2xl border border-blue-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {(() => {
                        const convAsigs = convalidacionAsignaturas ?? [];
                        ...
                        const STYLES = {
                          ...
                          perfil:      { bg: 'bg-purple-50 border-purple-100', code: 'text-purple-500 bg-purple-100', badge: 'text-purple-500 bg-purple-100 border-purple-200', label: `Perfil ${perfilProfesional}` },
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  <label key={perfil.id} className={`p-4 rounded-2xl border-2 transition-all cursor-pointer flex flex-col gap-1 ${formData.perfilProfesional === perfil.id ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-100 bg-gray-50 hover:border-gray-200'}`}>
                    <input type="radio" name="perfilProfesional" value={perfil.id} checked={formData.perfilProfesional === perfil.id} onChange={onChange} className="sr-only" />
