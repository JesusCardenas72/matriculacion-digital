...
export function calcularTasas(input: FeeInput): FeeResult | null {
  ...
  if (input.esPrimerAno) subtotalAdmin += fees.aperturaExpediente;

  let subtotalAcad = cursoBase;

  let pending1Cost = input.asignaturaPendiente1 ? fees.precioAsignatura * 1.2 : 0;
  let pending2Cost = input.asignaturaPendiente2 ? fees.precioAsignatura * 1.2 : 0;
  ...
  return {
    ...
    details: {
      ...
      aperturaExpediente: input.esPrimerAno ? fees.aperturaExpediente * multiplier : 0,
